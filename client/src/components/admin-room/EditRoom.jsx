...
function EditRoom () {
    ...
    const [updateInformation, setUpdateInformation] = useState({ roomTypeId:room.roomType.roomTypeId, statusId:room.statusId});
    ...
    const putUpdateInformation = async () => {
      try {
        const result = await axios.put("http://localhost:4000/status/update-room/" + room.roomId,{
          ...updateInformation
        }); 
        console.log(result);
      } catch (error) {
    ...
    const filterRoomTypeName = (search) => {
      return search
        ? roomType.filter((name) => name.roomTypeName.toLowerCase().includes(search))
        : roomType;
    }
      ...
      <div className="w-full h-screen bg-gray100 pb-10 relative top-0 left-0 z-0">
        ...
        <div className=" mt-10 mx-[60px] pt-10 pb-[60px] px-20 flex flex-col gap-20 bg-utilWhite rounded">
          ...
          <div>
            ...
            >
              {roomType.map((item) => item.roomTypeId === updateInformation.roomTypeId ? item.roomTypeName : "")}
              ...
              {isOpenRoomType[room.roomId] && (
                ...
                >
                  ...
                  <div className=" p-2 bg-white shadow-lg overflow-y-auto font-semibold">
                    <ul>
                      {filterRoomTypeName(searchRoomType).map((roomType, index) => (
                        <li
                          key={index}
                          onClick={() =>
                            setUpdateInformation({...updateInformation,roomTypeId:roomType.roomTypeId})
                          }
                    ...
                    <ul>
                      {filteredStatusNames.map((statusName) => (
                        <li
                          key={statusName}
                          onClick={() =>
                            setUpdateInformation({...updateInformation,statusId: getStatusIdByName(statusName)})
                          }
                          className={`${getStatusButtonColor(
                            statusName
